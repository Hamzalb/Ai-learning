...
import { Plus, Power, UserCheck } from 'lucide-react';
...
export default function SuperAdminPrincipalsPage() {
  ...
  const load = () => {
    Promise.all([superAdminAPI.getUsers({ role: 'principal' }), superAdminAPI.getSchools()])
  ...
  const handleCreate = async () => {
    try {
      await superAdminAPI.createUser({ ...form, role: 'principal' });
      ...
      <div className="space-y-5 max-w-5xl">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground">{principals.length} principals registered</p>
          </div>
          <Button onClick={() => setShowForm(!showForm)} className="btn-gradient gap-2"><Plus className="w-4 h-4" /> Add Principal</Button>
        </div>

        {showForm && (
          <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} className="glass-card p-5 space-y-4">
            <h3 className="font-bold text-foreground">Create Principal Account</h3>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-white/[0.06]">
                {['Principal', 'Email', 'School', 'Status', 'Actions'].map(h => (
                    ...
                    >
                      <td className="px-5 py-4">
                        <div className="flex items-center gap-3">
                          <div className="w-8 h-8 rounded-xl bg-amber-500/10 border border-amber-500/20 flex items-center justify-center text-amber-400 text-xs font-bold">{p.name.charAt(0)}</div>
                      ...
                      <td className="px-5 py-4">
                        <span className={cn('px-2.5 py-1 rounded-full text-xs font-medium border', p.isActive ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' : 'bg-red-500/10 text-red-400 border-red-500/20')}>
                          {p.isActive ? 'Active' : 'Inactive'}
                        </span>
                      </td>
                      <td className="px-5 py-4">
                        <button onClick={() => { superAdminAPI.toggleUser(p._id).then(load); }} className="p-1.5 rounded-lg hover:bg-white/[0.04] text-muted-foreground hover:text-foreground transition-colors">
