...
import { Plus, Search, Power, Users, X, Mail, Lock, Building2 } from 'lucide-react';
...
export default function SuperAdminStudentsPage() {
  ...
  const handleCreate = async () => {
    ...
    setSaving(true);
    try {
      const res = await superAdminAPI.createUser({ ...form, role: 'student' });
      const pwd = res.data?.data?.defaultPassword;
      toast.success(`Student created${pwd ? ` — password: ${pwd}` : ''}`);
      ...
      <div className="space-y-5 max-w-6xl mx-auto">
            ...
            <motion.div initial={{ opacity: 0, y: -12, scale: 0.98 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -12, scale: 0.98 }} transition={{ type: 'spring', stiffness: 300, damping: 28 }} className="glass-card p-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-5">
                <div>
                  ...
                  <input value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="Lara Haddad" className="input-field" />
                </div>
                <div>
                ...
                <div>
                  <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-1.5 block">Password <span className="normal-case font-normal text-muted-foreground/60">(default: Student@123)</span></label>
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1, type: 'spring', stiffness: 260, damping: 24 }} className="glass-card overflow-hidden">
          ...
          <table className="data-table">
            <thead><tr>{['Student', 'Email', 'School', 'Status', 'Actions'].map(h => <th key={h}>{h}</th>)}</tr></thead>
            <tbody>
              {loading ? Array.from({ length: 5 }).map((_, i) => (
                ...
                <tr><td colSpan={5}><div className="flex flex-col items-center py-16 gap-3">
                  <div className="icon-box-lg bg-violet-500/5 border border-violet-500/10"><Users className="w-6 h-6 text-violet-400/40" /></div>
                  ...
                  <motion.tr key={s._id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.04, type: 'spring', stiffness: 280, damping: 24 }}>
                    <td>
                      <div className="flex items-center gap-3">
                        <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-violet-500/20 to-purple-600/10 border border-violet-500/20 flex items-center justify-center text-violet-400 text-sm font-black shrink-0">{s.name.charAt(0).toUpperCase()}</div>
                    ...
                    <td>
                      <span className={s.isActive ? 'badge-success' : 'badge-error'}>
                        <span className={cn('w-1.5 h-1.5 rounded-full', s.isActive ? 'bg-emerald-400' : 'bg-rose-400')} />
                        {s.isActive ? 'Active' : 'Inactive'}
                      </span>
                    </td>
                    <td>
                      <button onClick={() => superAdminAPI.toggleUser(s._id).then(load)} className={cn('btn-icon', s.isActive ? 'hover:bg-amber-500/10 hover:text-amber-400 hover:border-amber-500/20' : 'hover:bg-emerald-500/10 hover:text-emerald-400 hover:border-emerald-500/20')}>
