...
import { Plus, Search, Power, BookOpen } from 'lucide-react';
...
export default function SuperAdminTeachersPage() {
  ...
  const handleCreate = async () => {
    try {
      await superAdminAPI.createUser({ ...form, role: 'teacher' });
      ...
      <div className="space-y-5 max-w-6xl">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground">{teachers.length} teachers across all schools</p>
          </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-white/[0.06]">
                {['Teacher', 'Email', 'School', 'Status', 'Actions'].map(h => (
                ...
                : teachers.map((t, i) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-5 py-4">
                        <button onClick={() => { superAdminAPI.toggleUser(t._id).then(load); }} className="p-1.5 rounded-lg hover:bg-white/[0.04] text-muted-foreground hover:text-foreground transition-colors">
