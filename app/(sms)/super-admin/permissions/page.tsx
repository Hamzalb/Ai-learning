...
const roleLabels: Record<string, string> = {
  super_admin: 'Super Admin', school: 'School', principal: 'Principal', teacher: 'Teacher', student: 'Student'
};
const roleColors: Record<string, string> = {
  super_admin: 'text-rose-400',   school: 'text-amber-400', principal: 'text-sky-400',
  teacher:     'text-emerald-400', student: 'text-violet-400'
};
    ...
    <SMSLayout allowedRoles={['super_admin']}>
      <div className="space-y-5 max-w-5xl mx-auto">
        ...
        <motion.div initial={{ opacity: 0, y: -12 }} animate={{ opacity: 1, y: 0 }} transition={{ type: 'spring', stiffness: 280, damping: 24 }}
          ...
          <div className="icon-box bg-gradient-to-br from-rose-500/20 to-rose-600/10 border border-rose-500/20 text-rose-400">
            <Shield className="w-5 h-5" />
          </div>
          <div>
            <h1 className="section-header">Role Permission Matrix</h1>
            <p className="section-subheader">Overview of what each role can do in the system</p>
          </div>
        </motion.div>
            ...
            <table className="w-full text-sm min-w-[560px]">
              <thead>
                <tr className="border-b border-white/[0.06]">
                  ...
                  {roles.map(r => (
                    <th key={r} className={`px-5 py-4 text-center text-xs font-bold uppercase tracking-wider ${roleColors[r]}`}>
                      {roleLabels[r]}
                    </th>
                  ))}
                </tr>
                  ...
                  <motion.tr key={perm.action} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.03 }}
                    ...
                    {roles.map(r => (
                      <td key={r} className="px-5 py-3.5 text-center">
                        {perm[r]
                          ? <div className="inline-flex w-6 h-6 rounded-full bg-emerald-500/15 border border-emerald-500/30 items-center justify-center">
                              <Check className="w-3 h-3 text-emerald-400" />
                            </div>
                          : <div className="inline-flex w-6 h-6 rounded-full bg-rose-500/10 border border-rose-500/20 items-center justify-center">
