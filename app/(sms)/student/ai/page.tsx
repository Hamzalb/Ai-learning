...
export default function StudentAIPage() {
  ...
  return (
    <SMSLayout allowedRoles={['student']}>
      <div className="flex flex-col h-[calc(100dvh-128px)] max-w-3xl mx-auto">
          ...
          {loading && (
            <div className="flex items-start gap-3">
              ...
              <div className="bg-white/[0.04] border border-white/[0.06] rounded-2xl px-4 py-3 flex gap-1.5">
                {[0, 1, 2].map(i => (
                  <div key={i} className="w-2 h-2 rounded-full bg-muted-foreground/40 animate-bounce"
        ...
        {/* Input */}
        <div className="flex gap-3 pt-4 border-t border-white/[0.06] shrink-0">
          <input
            value={input}
          ...
          <button onClick={send} disabled={loading || !input.trim()}
            className="w-11 h-11 rounded-2xl bg-gradient-to-br from-primary to-violet-600 flex items-center justify-center text-white shadow-lg shadow-primary/20 hover:shadow-primary/30 transition-all disabled:opacity-40 shrink-0">
