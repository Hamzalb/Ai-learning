...
export default function TeacherPayslipsPage() {
  ...
  useEffect(() => {
    teacherAPI.getPayslips().then(r => setPayslips(r.data.data.payslips || [])).finally(() => setLoading(false));
  }, []);
      ...
      <div className="space-y-5 max-w-3xl mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="glass-card p-5">
            ...
            <p className="text-3xl font-extrabold text-emerald-400 tabular-nums">${total.toLocaleString()}</p>
          </motion.div>
              ...
              <tbody>
                {loading ? Array.from({ length: 4 }).map((_, i) => (
                  ...
                  <tr><td colSpan={5}>
                    <div className="flex flex-col items-center py-16 gap-3">
                      <div className="icon-box-lg bg-emerald-500/5 border border-emerald-500/10">
                        <Wallet className="w-6 h-6 text-emerald-400/40" />
                      </div>
                  ...
                  <motion.tr key={p._id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.03 }}>
                    ...
                    <td className="font-bold text-emerald-400 tabular-nums">${p.amount.toLocaleString()} {p.currency}</td>
                    <td>
