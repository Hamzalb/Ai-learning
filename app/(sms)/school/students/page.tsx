...
export default function SchoolStudentsPage() {
  ...
  const load = () => schoolAPI.getStudents({ search }).then(r => setStudents(r.data.data.students)).finally(() => setLoading(false));
              ...
              {loading ? <tr><td colSpan={5} className="text-center py-12 text-muted-foreground">Loading...</td></tr>
                : students.map((s, i) => (
                  ...
                  >
                    ...
                    <td className="px-5 py-4 text-muted-foreground">{new Date(s.createdAt).toLocaleDateString()}</td>
                    <td className="px-5 py-4">
                      <button onClick={() => { schoolAPI.toggleStudent(s._id).then(load); }} className="p-1.5 rounded-lg hover:bg-white/[0.04] text-muted-foreground hover:text-foreground transition-colors">
