...
import { Plus, Power, Search, X, Mail, Phone, Lock, BookOpen } from 'lucide-react';
          ...
          {showForm && (
            <motion.div initial={{ opacity: 0, y: -12, scale: 0.98 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -12, scale: 0.98 }} transition={{ type: 'spring', stiffness: 300, damping: 28 }} className="glass-card p-6">
              ...
              <div className="flex items-center justify-between mb-5">
                <h3 className="font-bold text-foreground">Add Teacher</h3>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-5">
                ...
                <div>
                  ...
                  <div className="relative"><Phone className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground/60 pointer-events-none" />
            ...
            <tbody>
              {loading ? Array.from({ length: 4 }).map((_, i) => (
                ...
                <tr><td colSpan={5}><div className="flex flex-col items-center py-16 gap-3">
                  <div className="icon-box-lg bg-emerald-500/5 border border-emerald-500/10"><BookOpen className="w-6 h-6 text-emerald-400/40" /></div>
                  <p className="text-sm text-muted-foreground">No teachers yet</p>
                  <button onClick={() => setShowForm(true)} className="btn-gradient gap-2 mt-1"><Plus className="w-4 h-4" /> Add first teacher</button>
                </div></td></tr>
              ) : teachers.map((t, i) => (
                <motion.tr key={t._id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.04, type: 'spring', stiffness: 280, damping: 24 }}>
                  <td>
                    <div className="flex items-center gap-3">
                      <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-emerald-500/20 to-teal-600/10 border border-emerald-500/20 flex items-center justify-center text-emerald-400 text-sm font-black shrink-0">{t.name.charAt(0).toUpperCase()}</div>
